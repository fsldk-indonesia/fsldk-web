...
/** Config CmsIndexConfig<ShortLinkRequest> — antrean approval, bukan CRUD
 *  biasa, jadi TANPA createRoute/createLabel (tidak ada tombol "Tambah" —
 *  request datang dari publik) dan bulkDeleteEnabled selalu false (tidak
 *  ada konsep hapus untuk permintaan, lihat router.go RegisterRequestCMSRoutes
 *  yang memang tidak punya route delete/bulk-delete). Kolom "Tujuan" &
 *  "Kunci Diminta" ditandai sortable:false karena keduanya tidak ada di
 *  sortColumns backend (shortlinkrequest_service_impl.go). */
function buildShortlinkRequestIndexConfig(): CmsIndexConfig<ShortLinkRequest> {
  return {
    entityLabel: 'permintaan shortlink',
    guideCards: [
      { icon: 'clock', title: 'Antrean Permintaan', description: 'Permintaan shortlink dari publik masuk ke sini dengan status <strong>Pending</strong>, menunggu ditinjau admin.' },
      { icon: 'check', title: 'Setujui / Tolak', description: 'Klik ikon centang untuk menyetujui — shortlink langsung aktif — atau ikon silang untuk menolak beserta alasannya.' },
      ...
      { icon: 'eye', title: 'Detail Permintaan', description: 'Klik baris mana pun untuk melihat detail lengkap — kontak pemohon, catatan, hingga riwayat peninjauan.' },
    ],
    statusOptions: [
    ...
    columns: [
      ...
      { key: 'requestedKey', label: 'Kunci Diminta', sortable: false },
      { key: 'status', label: 'Status' },
      { key: 'createdDate', label: 'Diajukan' },
    ],
    defaultSort: { sortBy: 'createdDate', sortDir: 'desc' },
    rowIdKey: 'shortLinkRequestID',
    emptyIcon: 'clock',
    emptyTitle: 'Belum ada permintaan',
    emptyDescription: 'Permintaan shortlink dari publik akan muncul di sini.',
  };
}
...
@Component({
  ...
  styles: [`
    ...
    /* Selalu di-render (bukan @if) supaya transisi TUTUP juga kelihatan —
       pola sama seperti popup Pengguna/Role/Daftar Shortlink. */
    .modal-backdrop {
    ...
    /* Container scroll modal ini SEKALIGUS panel abu-abu (background tint +
       ...
       modul (.perm-mod) di atas .perm-list. Dipakai popup Tolak DAN Detail
       (keduanya berbagi class .modal-body yang sama). */
    .modal-body {
...
export class ShortLinkRequestIndexPage implements OnInit, ShortLinkRequestIndexView {
  ...
  @ViewChild(CmsIndexComponent) private table!: CmsIndexComponent<ShortLinkRequest>;
  ...
  viewTarget: ShortLinkRequest | null = null;

  readonly config = buildShortlinkRequestIndexConfig();
  ...
  openReject(r: ShortLinkRequest, event?: Event): void {
    event?.stopPropagation();
