...
**Konsep Artikel vs. Berita**: keduanya modul terpisah dengan struktur mirip, tapi Artikel punya model publikasi berbeda sejak field `articleContent` diganti `articleIntro` + ditambah `articleWriter`/`articleEditor`/`articlePdf` (lihat migrasi `0001_init.up.sql` di repo `fsldk-api`) — Artikel = pendahuluan singkat di web + naskah lengkap via PDF (byline penulis/editor eksplisit), sedangkan Berita tetap full-HTML di halaman detail seperti semula (`newsContent`, tanpa PDF). Jangan menyamakan pola keduanya saat menambah field baru ke salah satu modul.
